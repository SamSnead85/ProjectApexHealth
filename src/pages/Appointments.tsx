            ...
            >
                ...
                <button
                    ...
                    style={{
                        ...
                        color: showFilters ? '#818cf8' : 'rgba(255,255,255,0.7)', cursor: 'pointer', fontSize: '0.85rem'
                    }}
                >
                ...
                {(filterType !== 'all' || filterStatus !== 'all' || searchQuery) && (
                    <button
                        ...
                        style={{
                            ...
                            borderRadius: '8px', border: 'none', background: 'rgba(239,68,68,0.15)',
                ...
                {showFilters && (
                    <motion.div
                        ...
                        style={{
                            ...
                            borderRadius: '16px', background: 'rgba(10,15,26,0.6)', border: '1px solid rgba(255,255,255,0.06)'
                        }}
                    >
                        <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap' }}>
                            <div>
                                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem', display: 'block' }}>Type</span>
                                <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                                    {([['all', 'All'], ['in-person', 'In-Person'], ['video', 'Telehealth'], ['phone', 'Phone']] as const).map(([val, label]) => (
                                        <button key={val} onClick={() => setFilterType(val as typeof filterType)} style={{
                                            ...
                                            border: filterType === val ? '1px solid rgba(99,102,241,0.5)' : '1px solid rgba(255,255,255,0.08)',
                                            background: filterType === val ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.04)',
                                            color: filterType === val ? '#a5b4fc' : 'rgba(255,255,255,0.6)'
                                        }}>
                                            {label}
                                        </button>
                                    ))}
                            ...
                            <div>
                                <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem', display: 'block' }}>Status</span>
                                <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                                    {([['all', 'All'], ['confirmed', 'Upcoming'], ['completed', 'Past'], ['cancelled', 'Cancelled'], ['pending', 'Pending']] as const).map(([val, label]) => (
                                        <button key={val} onClick={() => setFilterStatus(val as typeof filterStatus)} style={{
                                            padding: '0.35rem 0.75rem', borderRadius: '8px', fontSize: '0.8rem', cursor: 'pointer',
                                            border: filterStatus === val ? '1px solid rgba(99,102,241,0.5)' : '1px solid rgba(255,255,255,0.08)',
                                            background: filterStatus === val ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.04)',
                                            color: filterStatus === val ? '#a5b4fc' : 'rgba(255,255,255,0.6)'
                                        }}>
                                            {label}
                                        </button>
                                    ))}
                    ...
                    >
                        <GlassCard style={{ borderRadius: '16px', padding: '1.5rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
                                ...
                                <button onClick={() => setShowBookForm(false)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', cursor: 'pointer' }}><X size={18} /></button>
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                                ...
                                <div>
                                    <label style={{ fontSize: '0.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.4rem', display: 'block' }}>Visit Type</label>
                                    <div style={{ display: 'flex', gap: '0.4rem' }}>
                                        {([['in-person', 'In-Person', MapPin], ['video', 'Telehealth', Video], ['phone', 'Phone', Phone]] as const).map(([val, label, Icon]) => (
                                            <button key={val} onClick={() => setBookForm({ ...bookForm, type: val })} style={{
                                                ...
                                                border: bookForm.type === val ? '1px solid rgba(6,182,212,0.5)' : '1px solid rgba(255,255,255,0.08)',
                                                background: bookForm.type === val ? 'rgba(6,182,212,0.12)' : 'rgba(255,255,255,0.04)',
                                                color: bookForm.type === val ? '#22d3ee' : 'rgba(255,255,255,0.6)'
                                            }}>
                                                <Icon size={13} /> {label}
                                            </button>
                                        ))}
                                ...
                                <div>
                                    <label style={{ fontSize: '0.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.4rem', display: 'block' }}>Notes</label>
                                    <input
                                        type="text"
            ...
            <div className="appointments__list">
                {filterAppointments(activeTab === 'upcoming' ? upcoming : past).length === 0 && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{
                        textAlign: 'center', padding: '3rem', color: 'rgba(255,255,255,0.4)', borderRadius: '16px',
                        background: 'rgba(10,15,26,0.4)', border: '1px solid rgba(255,255,255,0.06)'
                    }}>
